...
> **Status**: Approved — CD-GDD-ALIGN 2026-05-06 (APPROVE); /design-review fixes applied 2026-05-07 (RF-1~RF-8: emit ordering fix, HATCH_CELEBRATE non-interruptible, custom_blend API, typed Dictionaries, AC-13~16 added); RF-cross-1 applied 2026-05-07 (Interactions table: play_happy/confused trigger timing corrected to anim: Ink tags, not SELECTED_CORRECT/NOT_CORRECT events); **Incremental patch 2026-05-08**: added RECOGNIZE + SITTING states (14 logical states, 18 clips) for MainMenu GDD integration (MainMenu OQ-3 resolved); **/design-review RF-NEW-1~RF-NEW-9 applied 2026-05-08**: RECOGNIZE → NON_INTERRUPTIBLE_STATES (CD D1), SITTING trigger decoupled from fail_count → inactivity (CD D2), animation_completed(RECOGNIZE) added, Formulas/Dependencies/AC corrected; **B-3 applied 2026-05-08** (stop_recording_listen() timing clarified: SAVING phase signals, not RECORDING phase)
...
2. **状态枚举（内部）**：
   ...
   ```
   HAPPY 和 CONFUSED 各对应 3 个随机 clip（14 logical states，合计 18 clips）。enum 定义在 AnimationHandler 类内部，不暴露为 global enum——调用方通过语义方法调用，无需感知 enum。
...
5. **不可中断状态集合**：
   ```
   NON_INTERRUPTIBLE_STATES = [HATCH_CRACK, HATCH_EMERGE, HATCH_CELEBRATE, RECORDING_LISTEN, ENDING_WAVE, RECOGNIZE]
   ```
   `RECORDING_LISTEN` 只能通过 `stop_recording_listen()` 语义方法退出（不经过 `_transition()` guard）。`HATCH_CELEBRATE` 是孵化链最后一环，不可中断——被打断会导致 `hatch_sequence_completed` 永不发出，HatchScene 死锁。`RECOGNIZE` 是全游戏情感质量最高的动画（P4 要求），不可被任何事件打断——若 `chapter_load_failed` 在 RECOGNIZE 播放期间触发，AnimationHandler 静默拒绝 `play_confused()`；MainMenu 通过订阅 `animation_completed(RECOGNIZE)` 信号执行延迟调用（CD D1）。
...
6. **公开 API（语义方法）**：
   ...
   | `play_recognize() -> void` | RECOGNIZE（一次性→自动 MENU_IDLE） |
   | `play_sitting() -> void` | SITTING（循环，MainMenu 第 3 次连续失败） |
...
7. **`animation_finished` 处理**：`_ready()` 中连接 `_animation_player.animation_finished`，handler 按动画名执行链式跳转或回 IDLE：
   ...
   - `&"recognize"` → **先捕获 `var _completed_state := _current_state`**，再 `_transition(MENU_IDLE)`；最后 `animation_completed.emit(_completed_state)`（识别动画完成后自动回到 MainMenu 循环 idle，并发出信号供 MainMenu 执行延迟 confused 调用）
   ...
   - 循环状态（IDLE、HATCH_IDLE、MENU_IDLE、RECORDING_LISTEN、SITTING）在 AnimationPlayer 中设为 Loop，不产生 `animation_finished`
...
| `RECOGNIZE` | One-shot | `play_recognize()` | MENU_IDLE（auto） |
| `SITTING` | Loop | `play_sitting()` | 任意状态（如 `play_menu_idle()` 退出） |
...
| **RecordingInviteUI** | `stop_recording_listen()` | **B-3 修复**：SAVING 阶段信号到达后调用（`recording_saved` / `recording_failed` / `recording_interrupted`），不在 RECORDING 阶段中途调用——保证 T-Rex 举爪姿势在整个录音+保存期间持续可见 |
...
| **MainMenu** | `play_recognize()` | `_ready()` 时 `times_played > 0`（回归玩家识别动画） |
| **MainMenu** | `play_confused()` | `chapter_load_failed` 信号触发 → LOAD_ERROR 显示 |
| **MainMenu** | `play_sitting()` | `SITTING_INACTIVITY_THRESHOLD` 静置超时后（与 `_fail_count` 无关，CD D2） |
...
next_state(finished_clip) =
    ...
    MENU_IDLE + emit(animation_completed)         if finished_clip == &"recognize"
...
- `LOOP_CLIPS` = `{idle, hatch_idle, menu_idle, recording_listen, sitting}`（Loop 状态不产生 `animation_finished` 信号）
...
| 系统 | 依赖内容 | 契约 |
|------|---------|------|
| **Godot AnimationPlayer**（引擎内置） | `play(clip_name)`、`animation_finished` 信号、Loop 模式配置 | AnimationPlayer 节点必须以 `%AnimationPlayer` 唯一名称存在于同一场景树；18 个 clip 名须与 CLIP_MAP / VARIANT_MAP 一致；不直接依赖任何游戏系统 |
...
| **RecordingInviteUI** | `play_recording_invite()`、`stop_recording_listen()` | 录音邀请流程：先 `play_recording_invite()`（自动链到 RECORDING_LISTEN），**B-3 修复**：`stop_recording_listen()` 在 SAVING 阶段信号到达后调用（`recording_saved`/`recording_failed`/`recording_interrupted`），保证 RECORDING_LISTEN 姿势在录音+保存期间持续可见 |
...
| **MainMenu** | `play_menu_idle()` | MainMenu 进入时调用（`times_played == 0`）；`trex_recognize` 完成后自动链；LOAD_ERROR 退出后 |
| **MainMenu** | `play_recognize()` | `_ready()` 时 `times_played > 0`（回归玩家识别动画）；RECOGNIZE 在 NON_INTERRUPTIBLE_STATES 中，期间任何调用均被静默拒绝 |
| **MainMenu** | `play_confused()` | `chapter_load_failed` 直接触发（RECOGNIZE 已完成时）；或通过订阅 `animation_completed(RECOGNIZE)` 延迟触发（RECOGNIZE 播放期间 chapter_load_failed 触发时） |
| **MainMenu** | `play_sitting()` | `SITTING_INACTIVITY_THRESHOLD` 超时后（MainMenu 静置触发，与连续失败计数无关） |
...
| `NON_INTERRUPTIBLE_STATES` | `{HATCH_CRACK, HATCH_EMERGE, HATCH_CELEBRATE, RECORDING_LISTEN, ENDING_WAVE, RECOGNIZE}` | 设计决定 | 修改此集合会改变哪些动画可被打断；⚠️ 将 CONFUSED 加入此集合将违反 P2（失败路径不应阻断后续交互）；⚠️ HATCH_CELEBRATE 必须保留——移除会导致孵化链死锁；⚠️ RECOGNIZE 必须保留——移除会导致 chapter_load_failed 在识别动画中途打断情感最重要的时刻（P4 违反）
...
**SITTING 触发由 MainMenu 控制（不在 AnimationHandler 内部）**：`SITTING_INACTIVITY_THRESHOLD`（默认 30–60s，安全范围 15–120s）是 MainMenu 的 Tuning Knob，AnimationHandler 仅暴露 `play_sitting()` 语义方法；触发时机决策在 MainMenu 层，AnimationHandler 不感知超时逻辑。
...
| 约束 | 来源 | 说明 |
|------|------|------|
| 18 个 clip 名须与 CLIP_MAP / VARIANT_MAP 精确匹配 | 工程约束 | 命名不一致在运行时立即报错（AC-1 覆盖此验证） |
...
| Loop 状态（idle/hatch_idle/menu_idle/sitting）必须首尾无缝衔接 | 视觉质量 | 循环接缝在儿童长时间注视 NPC 时会被注意到 |
| story_advance clip 方向性明确（NPC 向前 / 向场景深处移动） | 叙事需要 | 无需语言，视觉传递"剧情推进了"的信号 |
| recognize clip 长度：1.5–3.0 秒，全游戏情感质量最高的动画 | P4 要求 + CD 门审 | 用整个身体表达记忆和喜悦（MainMenu GDD AC-22 CD 评审）；4 岁孩子在无提示情况下应能自发描述"恐龙认识我"；每次 App 回归仅播一次，是整个归属感论点的载体 |
| sitting clip 情感基调：休息/等待（坐下、收爪、垂头打盹），非消极 | P2 要求 | 禁止沮丧或自责情绪；SITTING 由静置超时触发（非连续失败），动画传递"等你回来"而非"你做错了" |
...
| # | 测试场景 | 期望结果 | 测试类型 |
|---|---------|---------|---------|
| AC-1 | 调用每个公开语义方法：`play_idle()`、`play_happy()`、`play_confused()`、`play_recording_invite()`、`stop_recording_listen()`、`play_hatch_idle()`、`play_hatch_crack()`、`play_story_advance()`、`play_menu_idle()`、`play_ending_wave()`、`play_recognize()`、`play_sitting()` | 每次 `AnimationPlayer.play()` 被调用，传入 CLIP_MAP / VARIANT_MAP 中存在的 clip 名；无 Godot 错误输出（共 12 个方法）| Unit |
...
| AC-17 | 调用 `play_recognize()`，等待动画完成 | `_current_state` 依次为 RECOGNIZE → MENU_IDLE；`animation_completed(AnimState.RECOGNIZE)` 在转入 MENU_IDLE 时发出 | Unit（mock animation_finished） |
| AC-18 | 在 RECOGNIZE 状态中调用 `play_confused()` | `_current_state` 保持 RECOGNIZE；`push_warning` 被调用；`AnimationPlayer.play()` 不被再次调用 | Unit |
| AC-19 | 在 RECOGNIZE 状态中调用 `play_happy()` | `_current_state` 保持 RECOGNIZE；`push_warning` 被调用；RECOGNIZE 完整播完 | Unit |
| AC-20 | 在 RECOGNIZE 状态中调用 `play_menu_idle()` | `_current_state` 保持 RECOGNIZE；guard 拦截；RECOGNIZE 正常完成后自动转 MENU_IDLE | Unit |
| AC-21 | 调用 `play_sitting()`，检查状态 | `_current_state == SITTING`；AnimationPlayer 以 Loop 模式播放 sitting clip | Unit |
| AC-22 | 在 SITTING 状态中调用 `play_menu_idle()` | `_current_state` 转 MENU_IDLE（SITTING 不在 NON_INTERRUPTIBLE_STATES，可被中断） | Unit |
| AC-23 | RECOGNIZE 完成后，`animation_completed` 携带状态参数 | 信号 payload `state == AnimState.RECOGNIZE`（非 IDLE、非 MENU_IDLE）；接收方可据此区分"识别完成"与"普通 one-shot 完成" | Unit |
