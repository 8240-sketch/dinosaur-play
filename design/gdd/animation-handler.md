...
> **Status**: Approved — CD-GDD-ALIGN 2026-05-06 (APPROVE)
...
3. **CLIP_MAP / VARIANT_MAP**：
   - `const CLIP_MAP: Dictionary` — 一对一状态到 clip 名映射（StringName）
   - `const VARIANT_MAP: Dictionary` — 一对多：`{ AnimState.HAPPY: [&"happy_1", &"happy_2", &"happy_3"], AnimState.CONFUSED: [&"confused_1", &"confused_2", &"confused_3"] }`；`_transition()` 内用 `Array.pick_random()` 选取随机变体
...
4. **`_transition(new_state: AnimState) -> void`**：所有状态切换的唯一内部入口。流程：
   ...
   c. `_animation_player.play(clip_name)`（blend_time 默认 0.0；2D 帧精灵禁止 blend）
   d. `_current_state = new_state`

5. **不可中断状态集合**：
   ```
   NON_INTERRUPTIBLE_STATES = [HATCH_CRACK, HATCH_EMERGE, RECORDING_LISTEN, ENDING_WAVE]
   ```
   `RECORDING_LISTEN` 只能通过 `stop_recording_listen()` 语义方法退出（不经过 `_transition()` guard）。
...
7. **`animation_finished` 处理**：`_ready()` 中连接 `_animation_player.animation_finished`，handler 按动画名执行链式跳转或回 IDLE：
   ...
   - 所有 happy_*/confused_* clip、`story_advance`、`ending_wave` → `_transition(IDLE)`；`animation_completed.emit(_current_state)`
...
| `HATCH_CELEBRATE` | One-shot | HATCH_EMERGE 完成后自动 | IDLE（auto）+ `hatch_sequence_completed` |
...
| `NON_INTERRUPTIBLE_STATES` | `{HATCH_CRACK, HATCH_EMERGE, RECORDING_LISTEN, ENDING_WAVE}` | 设计决定 | 修改此集合会改变哪些动画可被打断；⚠️ 将 CONFUSED 加入此集合将违反 P2（失败路径不应阻断后续交互） |

AnimationHandler 无数值调节参数——它是执行层，不产生游戏数值。blend_time 固定为 0.0（2D 帧精灵），不作为可调旋钮暴露。
