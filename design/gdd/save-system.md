...
2. **文件命名约定**：
   - 存档文件：`user://save_profile_{index}.json`，index ∈ {0 … MAX_PROFILES-1}
...
5. **v1 → v2 迁移（受控补缺）**：只添加缺失字段，绝不删除已有字段。迁移是幂等的——若 flush 失败，下次 load 会以相同结果重新迁移。v1 的已知词汇键：`["ch1_trex", "ch1_triceratops", "ch1_eat", "ch1_run", "ch1_big"]`。迁移时对每个词汇条目补填：`first_star_at: null`（若已存在则不覆盖）；`recording_path: null`（若已存在则不覆盖）。

6. **`_get_default_v2()`**：文件不存在时返回含完整 v2 结构的默认 Dictionary（不写磁盘）：`schema_version = CURRENT_SCHEMA_VERSION`，`last_saved_timestamp = ""`（ProfileManager 用此空字符串判断「从未写过盘」），`profile = {name: "", avatar_id: "", times_played: 0}`，`vocab_progress` 含所有 5 个词汇键（每键默认：`is_learned: false, gold_star_count: 0, first_star_at: null, recording_path: null`），`story_progress = {current_chapter_id: "", path_history: []}`。文件只在 `ProfileManager.create_profile()` 调用 `flush_profile()` 时才被实际写入磁盘。
...
| VocabStore | `flush_profile(index, data)` | 词汇状态批量更新后 |
| VoiceRecorder | `flush_profile(index, data)` | 录音完成，更新 recording_path |
| InterruptHandler | `flush_profile(index, data)` | App 进入后台 / 来电中断 |
...
```json
{
  ...
  "vocab_progress": {
    "ch1_trex":        { "is_learned": true,  "gold_star_count": 3, "first_star_at": "2026-04-15T10:23:41Z", "recording_path": "user://recordings/profile_0/ch1_trex.wav" },
    "ch1_triceratops": { "is_learned": false, "gold_star_count": 1, "first_star_at": "2026-05-01T09:15:00Z", "recording_path": null },
    "ch1_eat":         { "is_learned": false, "gold_star_count": 0, "first_star_at": null, "recording_path": null },
    "ch1_run":         { "is_learned": false, "gold_star_count": 0, "first_star_at": null, "recording_path": null },
    "ch1_big":         { "is_learned": false, "gold_star_count": 0, "first_star_at": null, "recording_path": null }
  },
...
| `recording_path` | VoiceRecorder | 透明载体，迁移时填 `null` |
...
| E6 | **index 越界**（< 0 或 >= MAX_PROFILES） | 设置 `INVALID_INDEX`，返回 `{}` | ProfileManager / VocabStore 在调用前自行验证 index |
| E7 | **recording_path 指向的 .wav 文件不存在** | SaveSystem 不验证 recording_path 文件有效性；透明载体 | VoiceRecorder 在播放前检查文件存在性；金星显示不受影响 |
| E8 | **首次安装/首次启动**，无任何存档文件 | `profile_exists(0)` 返回 `false`；`load_profile(0)` 返回 `_get_default_v2()`；不写磁盘 | ProfileManager 触发 HatchScene（`times_played == 0` 且 `name == ""`） |
...
| **VocabStore** | `flush_profile()` | 批量词汇更新后调用 flush；传入完整档案 dict |
| **VoiceRecorder** | `flush_profile()` | 录音完成后更新 recording_path 并调用 flush |
| **InterruptHandler** | `flush_profile()` | App 进入后台时紧急 flush；可能在任意时机调用 |

## Tuning Knobs

| 旋钮名 | 当前值 | 安全范围 | 影响 |
|--------|--------|---------|------|
| `MAX_PROFILES` | 2（MVP）→ 3（Vertical Slice） | 1–5 | 最大档案槽位数；超出后 ProfileManager 禁止新建档案 |
...
| AC-10 | index 越界 | `load_profile(-1)` 和 `load_profile(MAX_PROFILES)` 均返回 `{}`；`INVALID_INDEX` | Unit |
