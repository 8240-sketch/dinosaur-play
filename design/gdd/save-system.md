...
| VoiceRecorder | ~~`flush_profile(index, data)`~~ → `ProfileManager.flush()` | 录音完成，更新 recording_path（ProfileManager GDD 勘误：VoiceRecorder 调用 ProfileManager.flush()，不直接调用 SaveSystem） |
