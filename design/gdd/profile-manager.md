...
| **VoiceRecorder** | `get_section("vocab_progress")`, `flush()` | `vocab_progress` | 硬依赖（录音路径写入） |
...
| 信号名 | 参数 | 触发时机 | 订阅方 |
|--------|------|---------|--------|
| `profile_switch_requested(new_index: int)` | new_index: 目标档案 index（-1 = 正在删除活跃档案，无目标） | switch_to_profile 步骤 d；delete_profile(active) 步骤 a | StoryManager、VoiceRecorder（同步清理进行中状态） |
| `profile_switched(new_index: int)` | new_index: 已加载的新档案 index | switch_to_profile 步骤 g | GameRoot（确认切换完成，可进入游戏） |
...
2. **profile_switch_requested 同步执行时间预算**：当前 MVP 只有 StoryManager 和 VoiceRecorder 两个订阅者，同步清理可在单帧内完成。随系统增加，若订阅者达到 5 个以上，需评估是否引入异步切换协议（记录为潜在 v1.1 设计风险，不阻断本 GDD 批准）。
