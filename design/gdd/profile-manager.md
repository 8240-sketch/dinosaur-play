...
| `profile_switched(new_index: int)` | new_index: 已加载的新档案 index | switch_to_profile 步骤 g | GameRoot（确认切换完成，可进入游戏）、VocabStore（重新获取 `_vocab_data` 引用）、**MainMenu**（更新 `_active_index`、重算金星数、按 F-2 重执行动画逻辑——BLK-2 修复 2026-05-08） |
