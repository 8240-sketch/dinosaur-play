...
> **Status**: Approved — **B-2 applied 2026-05-08** (Interactions table: create_profile caller corrected to NameInputScreen); **S4-B1 applied 2026-05-08** (profile_switched subscriber: GameRoot checks is_first_launch() for HatchScene routing)
...
6. **`begin_session()` — times_played 写入点**：由 **MainMenu** 在「出发冒险！」发射序列步骤 c 显式调用一次——每次 App 访问的唯一一次，加载失败后重试不重复调用（MainMenu GDD Rule 11c/12）。ProfileManager 执行 `_active_data["profile"]["times_played"] += 1` 并立即调用 `flush()`（失败仅 push_error，不阻断流程）。在 `NO_ACTIVE_PROFILE` 状态下调用：push_warning，无操作。**GameRoot 不调用 `begin_session()`。**
...
13. **`get_profile_count() -> int`**：遍历 `0..MAX_SAVE_PROFILES-1`，统计 `SaveSystem.profile_exists(i) == true` 的数量，返回有效档案总数。范围：0 ≤ n ≤ `MAX_SAVE_PROFILES`（3）。仅做委托统计，ProfileManager 不缓存本地计数——每次调用均实时查询 SaveSystem。
...
| MainMenu → ProfileManager | `get_profile_count()` | 切换器可见性判断（F 节新增方法） |
| MainMenu → ProfileManager | `begin_session()` | 「出发冒险！」发射序列步骤 c（每次 App 访问唯一一次）|
| MainMenu → ProfileManager | `switch_to_profile(index)` | 档案切换 |
| NameInputScreen → ProfileManager | `create_profile(index, name, avatar_id)` | **B-2 修复**：NameInputScreen 是 `create_profile()` 的唯一合法调用方（NameInputScreen GDD Rule 8）；MainMenu 不直接调用 `create_profile()` |
...
| VoiceRecorder → ProfileManager | `get_section("vocab_progress")` | 更新 recording_paths |
...
| (signal) → VocabStore | `profile_switch_requested` | 同步清除 `_vocab_data` 和 `_session_counters` 引用 |
| (signal) → GameRoot | `profile_switched` | 确认切换完成，可进入游戏（switch_to_profile 步骤 g） |
| (signal) → VocabStore | `profile_switched` | 重新获取新档案的 `_vocab_data` 引用 |
| (signal) → GameRoot | `active_profile_cleared` | 跳转档案选择界面 |
| (signal) → VoiceRecorder | `active_profile_cleared` | reason == "user_deleted" 时，递归删除 `user://recordings/profile_{deleted_index}/` 目录 |
...
| `parent_map_hint_dismissed` | bool | `true` / `false` | `false` = 家长尚未访问成长地图（默认值）；`true` = 首次关闭 ParentVocabMap 后由其写入，MainMenu 不再显示发现提示标记。**SaveSystem `_migrate_to_v2` 须预填此字段（默认 `false`），防止旧存档读取异常。写权限：ParentVocabMap（通过 `get_section("profile")` 直接写入并调用 `flush()`）** |
...
**前置条件：** 当前状态为 `ACTIVE`；由 **MainMenu** 在「出发冒险！」发射序列调用一次/App 访问（MainMenu GDD Rule 11c）。
...
|------|-----------|------------|---------|
| **GameRoot** | `switch_to_profile(index)`, `is_first_launch()` | — | 硬依赖（档案激活和首次启动判断）；`begin_session()` 由 **MainMenu** 调用，不在 GameRoot 职责内（Core Rule 6） |
| **GameRoot** | 订阅 `profile_switched`，订阅 `active_profile_cleared` | — | 信号订阅（导航控制）；**S4-B1 修复**：`profile_switched` 收到后检查 `is_first_launch()`——`times_played == 0` 时路由至 HatchScene（新档案孵化序列），`times_played > 0` 时路由至 MainMenu |
...
| **VoiceRecorder** | `get_section("vocab_progress")`, `flush()` | `vocab_progress` | 硬依赖（录音路径写入）；字段分工：VocabStore 写 `is_learned`/`gold_star_count`/`first_star_at`；VoiceRecorder 仅写 `recording_paths`；两者字段无交叠 |
...
| `profile_switched(new_index: int)` | new_index: 已加载的新档案 index | switch_to_profile 步骤 g | GameRoot（确认切换完成，可进入游戏；**S4-B1 修复**：检查 `is_first_launch()`——若 `times_played == 0` 则路由至 HatchScene 而非 MainMenu，防止新档案跳过孵化序列）、VocabStore（重新获取 `_vocab_data` 引用）、**MainMenu**（更新 `_active_index`、重算金星数、按 F-2 重执行动画逻辑——BLK-2 修复 2026-05-08） |
| `active_profile_cleared(reason: String)` | reason: `"user_deleted"` — 主动删除；`"load_failed"` — 加载失败降级 | delete_profile(active) 步骤 d；EC-1 加载失败降级 | GameRoot（跳转档案选择界面；可据 reason 显示不同提示）；VoiceRecorder（reason == "user_deleted" 时，递归删除 `user://recordings/profile_{deleted_index}/` 目录） |
