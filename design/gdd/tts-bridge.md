...
2. **TtsProvider 合约**（Path A — 见 ADR-0002-TTS-PROVIDER）：每个 AI TTS 提供商须实现以下接口（`@abstract class TtsProvider extends RefCounted`）：
   ...
   TtsProvider **不发出信号**；TtsBridge 统一管理所有 HTTP I/O、降级逻辑和信号发出。`ProviderError`：`CONFIGURATION_ERROR = 0`（永久性，如 401/403 Key 无效）和 `TRANSIENT_ERROR = 1`（临时性，如超时、502/503）。HTTP 400 由 TtsBridge 直接处理（push_error + Tier 2，不经 `classify_error`，不计入连续失败计数器）。
